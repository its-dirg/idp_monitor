...
class RResponse():
    """
    A Response class that behaves in the way that mechanize expects.
    ...
    def __init__(self, resp):
        ...
        if isinstance(resp.text, unicode):
            if resp.encoding == "UTF-8":
                self.text = resp.text.encode("utf-8")
            else:
                self.text = resp.text.encode("latin-1")
        else:
            self.text = resp.text
...
class Interaction(object):
    ...
    def pick_interaction(self, _base="", content=""):
        """
        Based on what the form looks like and from where the result came,
        pick which action to perform.

        :param _base: The URL from which the response came from
        :param content: The content of the page
        :return: The interaction that matched or throw an exception if none
            did.
        """
        unic = content
        if content:
...
class Action(object):
    ...
    def __call__(self, httpc, conv, location, response):
        ...
        _args.update({"location": location, "conv": conv})
